...
- 维护项目状态文档（`status.md`、`plan.md`、巡检日志），对人类负责
...
- ❌ **不将测试文件修改和业务代码修改合并到同一任务**（测试代码 → QA Agent；业务代码 → 对应模块 Agent，必须分开派发）
...
| 项目状态文档 | `docs/project/status.md` | 定期更新，含阻塞项和责任方 |
...
6. 更新 status.md、写入巡检日志、更新 log.md 索引
```
...
```
当测试执行发现问题时，必须按性质分开派发，不得合并到同一任务：
  - 测试代码问题（_test.go 文件需新增/修改）      → 派发给 QA Agent
  - 业务代码 Bug（_test.go 以外的 .go 文件需修改） → 派发给对应模块的开发 Agent

派发时必须明确：
  1. 任务 ID、任务标题、负责 Agent
  2. 涉及的文件路径（测试文件 vs 业务文件分开列举）
...
| 越界行为 | 纠偏方式 |
|---------|---------|
| QA 报告中包含代码架构评审意见 | 将架构问题转交 Tech Lead，要求 QA 补充测试视角的评审 |
...
- `.claude/harness/state.json` — 状态快照更新
