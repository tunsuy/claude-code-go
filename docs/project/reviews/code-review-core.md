...
|---------|------|---------|------|---------|------|
| P0-CR-1 | P0 | `e.messages` 未写回，`GetMessages()` 永远返回过期数据 | ✅ 已修复 | 2026-04-03 | `runQueryLoop` 头部增加 `defer` 块，在 `mu.Lock()` 保护下将本轮最终 `messages` 完整拷贝回 `e.messages` |
| P0-CR-2 | P0 | `BaseTool.InterruptBehavior()` 默认值 Cancel 应为 Block | ✅ 已修复 | 2026-04-03 | `base.go` 第 39 行已改为 `return InterruptBehaviorBlock` |
| P0-CR-3 | P0 | `input_json_delta` 事件错误发送 `MsgTypeToolUseStart` | ✅ 已修复 | 2026-04-03 | `query.go` 中 `case "input_json_delta"` 已改用 `MsgTypeToolUseInputDelta`；`msg.go` 已声明该常量 |
| P0-CR-4 | P0 | `buildRequest` 以 `nil` 调用 `Description(nil, nil)` 可触发 nil panic | ✅ 已修复 | 2026-04-03 | `buildRequestWithModel` 中先构造合法 `var permCtx tool.PermissionContext`（若 `ToolUseContext != nil` 则赋值）再传入 |
| P0-CR-5 | P0 | `FallbackModel` 无条件替换主模型，主模型永远不会被使用 | ✅ 已修复 | 2026-04-03 | 主请求始终用 `e.model`；仅当 `apiErr.Kind == api.ErrKindContextWindow` 时才切换 fallback |
| P0-CR-6 | P0 | 压缩流水线（snip → micro → auto）从未在查询循环中调用 | ✅ 已修复 | 2026-04-03 | `query.go` 已按设计规格顺序插入三步压缩；`engine.go` `New()` 中 compactor 已初始化 |

> **本层评审通过，通知 PM。**
> 核心层 6 项 P0 均已正确修复，代码质量符合合并标准。P1/P2 技术债按原计划后续迭代跟进。
