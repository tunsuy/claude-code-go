...
| 阶段 | 内容 | 负责方 |
|------|------|-------|
| **M0：启动** | 架构设计完成（已完成）；PM 与各 Agent 制定项目计划表 | Tech Lead + PM + 全体 |
| **M1：并行开发** | 所有 Agent 同时认领任务并行开发；依赖未就绪时 TODO 标记，PM 持续跟踪解锁 | 全体开发 Agent + QA |
| **M2：集成稳定** | 补全所有 TODO、模块集成、集成测试、回归测试 | Tech Lead + 全体 + QA |
| **M3：交付** | 可运行 Go 版本 + 完整测试报告 + 文档 | PM 主导 |

### 并行开发模式

所有开发 Agent 在项目计划确定后**同时启动**，不等待依赖层完成。
...
**依赖处理原则**：
- 依赖未就绪时，用 `// TODO(dep): 等待 <模块> 就绪后补全` 标记，继续推进其他工作
- PM 跟踪依赖状态，一旦依赖模块完成，立即通知相关 Agent 回填 TODO
- Agent 不主动等待，依赖阻塞时切换到不依赖的任务
